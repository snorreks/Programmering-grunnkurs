...
class Grafikken2 extends GLCanvas implements GLEventListener{
	...
	public void drawGLScene(GLAutoDrawable glDrawable){
		...
		gl.glVertex3f(1.0f, -1.0f, 1.0f);//h�yre bak
		gl.glVertex3f(1.0f, -1.0f, -1.0f);//h�yre framme
		gl.glVertex3f(-1.0f, -1.0f, -1.0f); //tilbake til start

		gl.glVertex3f(-1.0f, 1.0f, -1.0f); // g�r opp
		gl.glVertex3f(-1.0f, 1.0f, 1.0f); //g�r bak
		gl.glVertex3f(-1.0f, -1.0f, 1.0f); //g�r ned
		gl.glVertex3f(-1.0f, 1.0f, 1.0f); //g�r opp
		gl.glVertex3f(1.0f, 1.0f, 1.0f); //til h�yre
		gl.glVertex3f(1.0f, -1.0f, 1.0f); //g�r ned
		gl.glVertex3f(1.0f, 1.0f, 1.0f); //opp igjen
		gl.glVertex3f(1.0f, 1.0f, -1.0f); //g�r fram
		gl.glVertex3f(1.0f, -1.0f, -1.0f);//g�r ned
		gl.glVertex3f(1.0f, 1.0f, -1.0f);//g�r opp
