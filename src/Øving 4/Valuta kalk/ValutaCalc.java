...
class ValutaCalc{
	...
	//Les inn data fra bruker
		String Lest = showInputDialog("Skriv inn beløp: ");
		double beløp = Integer.parseInt(Lest);
		if(beløp == 0){showMessageDialog(null,"Skrev ikke inn beløp"); break;}
	//Valuta
	...
	//Regn ut sum og gi svaret tilbake til bruker
		...
		if (response1 == 0 && response2 == 0){ 									//til USD
			sum += USD.tilNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " Kroner (NOK)");
		} else if (response1 == 0 && response2 == 1){							//til Euro
			sum += Euro.tilNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " Kroner (NOK)");
		} else if (response1 == 0 && response2 == 2){							//til Svensk
			sum += Svensk.tilNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " Kroner (NOK)");
		} else if(response1 == 1 && response2 == 0){							//fra USD
			sum += USD.fraNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " " + USD.getName());
		} else if(response1 == 1 && response2 == 1){							//fra Euro
			sum += Euro.fraNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " " + Euro.getName());
		} else if(response1 == 1 && response2 == 2){							//fra Svensk
			sum += Svensk.fraNorsk(beløp);
			showMessageDialog(null,"Beløpet er: " + sum + " " + Svensk.getName());
		} else {run = false;}
